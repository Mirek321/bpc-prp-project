...
{
    if (!msg->data.empty()) {
        // Take the first detected marker (assuming only one per intersection)
        last_aruco_id_ = msg->data[0];
        RCLCPP_INFO(this->get_logger(), "ArUco detected: %d", last_aruco_id_);
    } 
}
