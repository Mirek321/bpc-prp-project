...
public:
    ...
    RosExampleClass(const rclcpp::Node::SharedPtr &node, const std::string &topic, double freq, std::function<double(double)> func)
        : node_(node), start_time_(node_->now()), func_(func) {
